...
with open('kadınlar.csv', 'r') as csvfile:
...
def fix(full_name, gender_dict, male_counter, female_counter, region_name, donem): # bizim bulduklarimiza gore kadin erkek puan veriyor
    try:
...
for term in range(27):
    terms[term + 1] = [0,0]

print(terms)
for i in range(len(tr_female_counter)):
    print(tr_female_counter[i], tr_male_counter[i])
...
header = ["Term", "Year", "Female MP Count", "Male MP Count"]
with open(folder_path + "/formatted/" + 'terms.csv', 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(header)
        for i in range(27):
            writer.writerow([i+1, year_list[i].split(" ")[1], terms[i+1][0], terms[i+1][1]])
print()
print()
print()
print()
for i in range(len(termcount)):
    print(i+1, termcount[i])